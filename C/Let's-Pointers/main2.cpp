...
//there are two type of function 1.call by value (simple) 2.call by refrence (in which pointers are passed as arguments)
...
//for simple functions (not call by reference) calling function have actual arguments whare the called function have formal arguments(parameters)
...
int main(){
    ...
    //p=arr is neccessory because we can't use (arr) directly as it couse error if we use arr++ or etc
    for(int i=0;i<5;i++){
