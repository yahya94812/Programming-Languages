...
int* Add(int*a, int*b){//bad practice
    int c = *a+*b;
    //insted use heap//very good practice and no threads
...
int main(){
    ...
    int x = 0, y = 0;//couse warning and not guaranteed espacilly after calling other functions as the same portion of stack is taken by other functions
    int *padd=Add(&x,&y);
    cout<<"x = "<<x<<endl;
    ...
    for(int i=0;i<varSize;i++){
        cout<<p[i]<<" ";//return garbage values and p[3] is 23 and there is no garunte for it if we call any function
    }
    cout<<endl;

    //there fore set p to NULL for preventing errors
    ...
    //never declare any pointers in callind function because if that function is exicuted and returns then all the values that are pointed by that pointer will be cleared (or stay there only) if any other function is take the same stack portion which the previous function was taken then all values will be cleared
