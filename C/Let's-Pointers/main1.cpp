...
int main(){
    ...
    int* ptr1,* ptr2; //consider (int*) the data type for declaring integer pointer
    ...
    cout<<"value of the above pointers is :"<<endl<<*ptr1<<" and "<<*ptr2<<endl;  

    *ptr1=3;//ptr1 value is not change but p1 value is changed
    cout<<ptr1<<" and "<<ptr2<<endl;
    //accessing value at pointer using differencing operator (*)
    ...
    int* p;
    cout<<p<<endl;//garbage address as a value if p
    cout<<*p<<endl;//value {may be any usefull value of os or unoccupied} at garbage address
    ...
    cout<<int(*ptrc)<<" is the value in int"<<endl;//out 1, type casting and it is the value of first byte
    ptrc++;
    cout<<int(*ptrc)<<" is the value at second byte "<<endl;//out 4

    //void pointer or genric pointer
    int iv;
    void* pv;
    int *nv;
    nv=&iv;
    ...
    // temp=&x;
    //we can use multiple (*) like **** but we can't use multiple (&) because consider &&x this means find or give the variable address that store the address of x, in whole memory (it can be done with searching algorithm)
    //where the multiple (*) which mean value at pointer which is again pointer to some value and so on 
