...
int sumOfElements(int arr[], int size) // we can also use int* arr insted of arr[] but arr[] increas readablity
{
...
void pointerPrint(int *ptr){//you can also use ptr[]
    cout<<"the pointer is :"<<ptr[0];//you can also use *ptr
    return;
}
int main()
{
